...
def nyc_route_traveler_euler(t0, h):
  ...
  h_list = [t0]                               #skapar listor som endast innehåller initialvärden
  x_list = [x]
  speed_list = [route_nyc(t, x)[0][0]]

  #EULERS METOD (EXPLICIT EULER)
  while x < 60:                              #loopar så länge vi befinner oss innanför ruttens totala längd
    if x + h * route_nyc(t, x)[0][0] > 60:      #om det sista steget överstiger ruttens totala längd anpassas steglängden
      h = (60 - x) / route_nyc(t, x)[0][0]
    x = x + h * route_nyc(t, x)[0][0]
    t = t + h
    h_list.append(t)                          #lägger till genererade värden till listorna
    x_list.append(x)
    speed_list.append(route_nyc(t, x)[0][0])
  
  time_h = np.array(h_list)          #gör om listorna till numpy-arrayer
  distance_km = np.array(x_list)
  speed_kmph = np.array(speed_list)
...
#Följande kod skapar isolerade plottar av ruttfärderna med start kl 04.00 (orange) respektive kl 09.30 (rosa)

import matplotlib.pyplot as plt

time_h_4 = nyc_route_traveler_euler(4, 0.01)[0]
distance_km_4 = nyc_route_traveler_euler(4, 0.01)[1]
time_h_9_5 = nyc_route_traveler_euler(9.5, 0.01)[0]
distance_km_9_5 = nyc_route_traveler_euler(9.5, 0.01)[1]

plt.subplot(2, 1, 1)
plt.plot(time_h_4, distance_km_4, color = 'darkorange', label = 'Ruttfärd med start kl. 04:00')
plt.xlabel('Time [hour of day]',fontsize=9)
plt.ylabel('Distance [km]',fontsize=9)
plt.title('Distance over time',fontsize=11)
plt.legend()

plt.subplot(2, 1, 2)
plt.plot(time_h_9_5, distance_km_9_5, color = 'fuchsia', label = 'Ruttfärd med start kl. 09:30')
plt.xlabel('Time [hour of day]',fontsize=9)
plt.ylabel('Distance [km]',fontsize=9)
plt.title('Distance over time',fontsize=11)
plt.legend()
